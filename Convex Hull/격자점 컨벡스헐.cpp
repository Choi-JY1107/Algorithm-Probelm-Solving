...
int main() {
	int n, cnt, testcase, startPoint;
	coor temp, start;
	vector <int> ans;
	cin >> testcase;
	
	while(testcase--){
		coors.clear();ans.clear();
		start = {100,-100};
		...
		for(cnt=0;!convex.empty();cnt++,ans.push_back(convex.top().index),convex.pop());
		cout << cnt << endl;
		
		for(int i=0;i<cnt;++i){
			if(start.y < coors[ans[i]].y || 
				(start.y == coors[ans[i]].y && start.x > coors[ans[i]].x)){
					start = coors[ans[i]];
					startPoint = i;
				}
		}
		for(int i=startPoint; i>=0; --i) cout << coors[ans[i]].x << " " << coors[ans[i]].y << endl;
		for(int i=cnt-1; i>startPoint; --i) cout << coors[ans[i]].x << " " << coors[ans[i]].y << endl;
	}
